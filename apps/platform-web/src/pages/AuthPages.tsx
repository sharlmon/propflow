...
export function LoginPage() {
  ...
  if (user)
    return <Navigate to={user.role === 'landlord' ? '/propflow/dashboard' : '/keja/dashboard'} replace />;
  async function submit(values: LoginValues) {
    setSubmitError('');
    try {
      const signedIn = await login(values);
      const requested = (location.state as { from?: { pathname?: string } } | null)?.from?.pathname;
      navigate(requested || (signedIn.role === 'landlord' ? '/propflow/dashboard' : '/keja/dashboard'), {
        replace: true,
      });
    } catch (error) {
...
export function RegisterPage() {
  ...
  async function submit(values: RegisterValues) {
    ...
    try {
      ...
      navigate(created.role === 'landlord' ? '/propflow/dashboard' : '/keja/dashboard');
    } catch (error) {

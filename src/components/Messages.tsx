...
import { Search, Send, Paperclip, MoreVertical, Phone, Video, Info, User as UserIcon, ShieldCheck, MoreHorizontal, MessageSquare } from 'lucide-react';
...
const CONVERSATIONS = [
    { id: 'landlord', name: 'Musa Omari (Landlord)', lastMsg: 'I have checked your maintenance request.', time: '10:30 AM', unread: 2, unit: 'Property Owner', type: 'Landlord' },
    { id: 'mgmt', name: 'Amani Management', lastMsg: 'The utility invoice is ready.', time: 'Yesterday', unread: 0, unit: 'Main Office', type: 'Support' },
    { id: 't2', name: 'Bob Smith', lastMsg: 'When will the AC be fixed?', time: 'Yesterday', unread: 0, unit: 'Amani Heights #102', type: 'Tenant' },
    { id: 't3', name: 'Charlie Davis', lastMsg: 'Thanks for the reminder.', time: '2 days ago', unread: 0, unit: 'Campus #5', type: 'Tenant' },
];
    ...
    return (
        <div className="bg-white rounded-[40px] shadow-xl shadow-slate-100 border border-slate-100 h-[calc(100vh-14rem)] flex overflow-hidden animate-in fade-in duration-500">
            ...
            <div className="w-full md:w-80 border-r flex flex-col bg-slate-50/50">
                ...
                <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                    {filteredConversations.map(conv => (
                        ...
                        >
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${conv.type === 'Landlord' ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-500'}`}>
            ...
            <div className="hidden md:flex flex-1 flex-col bg-white">
                ...
                <div className="p-6 border-b flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${selectedConv.type === 'Landlord' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                            <UserIcon className="w-6 h-6" />
                        </div>
                        <div>
                    ...
                    <div className="flex gap-2">
                        <button className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-2xl transition-all"><Phone className="w-5 h-5" /></button>
                        <button className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-2xl transition-all"><MoreHorizontal className="w-5 h-5" /></button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/20">
                    <div className="flex justify-center">
                        <span className="text-[10px] font-black text-slate-400 bg-white px-4 py-1.5 rounded-full uppercase tracking-widest border border-slate-100 shadow-sm">May 31, 2024</span>
                    </div>
                    ...
                    <div className="flex items-start gap-4 max-w-[70%]">
                        ...
                        <div className="bg-white p-5 rounded-[24px] rounded-tl-none border border-slate-100 shadow-sm">
                            <p className="text-sm text-slate-700 leading-relaxed font-medium">Hello {user.name.split(' ')[0]}, I just wanted to confirm the receipt of your maintenance request for the leaky sink. We've assigned a plumber for tomorrow morning.</p>
                            <span className="text-[10px] text-slate-400 font-black mt-3 block uppercase tracking-wider">10:30 AM • Sent by Landlord</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4 max-w-[70%] ml-auto flex-row-reverse">
                        ...
                        <div className="bg-indigo-600 p-5 rounded-[24px] rounded-tr-none shadow-xl shadow-indigo-100">
                            <p className="text-sm text-white leading-relaxed font-medium">Thank you Musa! Tomorrow morning works perfectly. I'll be home to let them in.</p>
                            <span className="text-[10px] text-indigo-200 font-black mt-3 block uppercase tracking-wider text-right">10:35 AM • Delivered</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 bg-white">
                    <form
                        ...
                        onSubmit={(e) => { e.preventDefault(); setMsgInput(''); }}
                    >
                        <button type="button" className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-2xl transition-all">
            ...
            <div className="md:hidden flex-1 flex flex-col items-center justify-center p-8 text-center bg-white">
                ...
                <p className="text-sm text-slate-400 font-medium">Select a thread from the list to start messaging.</p>
            </div>
        </div>
    );
};

...
import { Plus, Search, Filter, Wrench, MoreVertical, MessageSquare, X, Hash, AlertTriangle } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[32px] w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleCreateTicket} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Unit Number</label>
                            ...
                            <div className="p-4 bg-amber-50 rounded-2xl flex gap-3 border border-amber-100">
                                ...
                                <p className="text-xs text-amber-800 leading-relaxed font-medium">
                                    If this is a life-threatening emergency (fire, flood, gas leak), please call emergency services immediately.
                                </p>
                            </div>
                        ...
                        {loadingTriage ? (
                            ...
                            <div className="space-y-5">
                                <div className="p-5 bg-indigo-50 rounded-2xl border border-indigo-100">
                                    <p className="text-[10px] uppercase font-black text-indigo-400 tracking-wider mb-2">Suggested Priority</p>
                                    <p className="text-xl font-black text-indigo-900">{triageResult?.priority}</p>
                                </div>
                                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                    <p className="text-[10px] uppercase font-black text-slate-400 tracking-wider mb-2">AI Recommendation</p>
                                    <p className="text-slate-700 leading-relaxed font-medium">{triageResult?.advice}</p>
                                </div>
                                <div className="flex gap-3 mt-6">
                                    <button onClick={() => setShowTriage(null)} className="flex-1 py-4 border border-slate-200 rounded-2xl font-black text-sm hover:bg-slate-50 transition-colors">Dismiss</button>
                                    <button className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-100">Assign Vendor</button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {tickets.map(ticket => (
                    <div key={ticket.id} className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all relative group">
                        <div className="flex justify-between items-start mb-6">
                            <span className={`px-3 py-1 text-[10px] font-black rounded-full uppercase tracking-widest ${ticket.priority === 'Emergency' ? 'bg-red-100 text-red-700' :
                        ...
                        <p className="text-sm text-slate-500 line-clamp-2 mb-6 font-medium leading-relaxed">{ticket.description}</p>
                        ...
                        <div className="flex items-center justify-between border-t border-slate-50 pt-6">
                            <div className="flex -space-x-2">
                                {[1, 2].map(i => <img key={i} src={`https://picsum.photos/32/32?sig=${ticket.id}${i}`} className="w-8 h-8 rounded-full border-2 border-white" alt="Vendor" />)}
                            </div>

...
function AuthCard({ children, title, intro }: { children: React.ReactNode; title: string; intro: string }) {
  return (
    <main className="grid min-h-[calc(100vh-65px)] place-items-center p-4">
      <section className="w-full max-w-md rounded-2xl border border-blue-100 bg-white p-6 shadow-lg shadow-blue-950/5 sm:p-8">
...
export function LoginPage() {
  ...
  if (user)
    return <Navigate to={user.role === 'landlord' ? '/landlord/dashboard' : '/renter/dashboard'} replace />;
  async function submit(values: LoginValues) {
    setSubmitError('');
    try {
      const signedIn = await login(values);
      const requested = (location.state as { from?: { pathname?: string } } | null)?.from?.pathname;
      navigate(requested || (signedIn.role === 'landlord' ? '/landlord/dashboard' : '/renter/dashboard'), {
        replace: true,
      });
    } catch (error) {
  ...
  return (
    <AuthCard title="Welcome back" intro="Sign in to continue to your property or renter workspace.">
...
export function RegisterPage() {
  ...
  const {
    ...
    defaultValues: { role: 'renter', phone: '' },
  });
  if (user)
    return <Navigate to={user.role === 'landlord' ? '/landlord/dashboard' : '/renter/dashboard'} replace />;
  async function submit(values: RegisterValues) {
    ...
    try {
      ...
      navigate(created.role === 'landlord' ? '/landlord/dashboard' : '/renter/dashboard');
    } catch (error) {
      if (error instanceof ApiError) {
        Object.entries(error.fields).forEach(([name, message]) =>
          setError(name as keyof RegisterValues, { message }),
        );
        setSubmitError(error.message);
      } else setSubmitError('Unable to create the account.');
    }
  }
  return (
    <AuthCard title="Create your account" intro="Choose the experience you need today.">
      <form className="space-y-5" onSubmit={handleSubmit(submit)} noValidate>
        {submitError ? (
          <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-red-800">
            {submitError}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="rounded-xl border p-3">
              <input type="radio" value="renter" {...register('role')} /> <span className="ml-2">Renter</span>
            </label>
            <label className="rounded-xl border p-3">

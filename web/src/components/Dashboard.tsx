...
export const Dashboard: React.FC<DashboardProps> = ({ user, properties, tenants, setActiveTab }) => {
  ...
  const totalRent = FINANCIALS.filter((f) => f.category === 'Rent').reduce(
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500">
          Your property portfolio is performing <span className="text-emerald-600 font-bold">well</span>{' '}
          today.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-7 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1.5 text-xs font-bold text-slate-500 bg-slate-50 rounded-lg">
                6 Months
              </button>
        ...
        <div className="bg-white p-7 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
          ...
          <button
            ...
            className="mt-8 w-full py-3 text-sm font-bold text-indigo-600 bg-indigo-50 rounded-2xl hover:bg-indigo-100 transition-colors"
          >
    ...
    <div className="bg-indigo-700 p-8 rounded-[40px] text-white relative overflow-hidden shadow-2xl shadow-indigo-200">
      <div className="relative z-10">
        ...
        <p className="text-indigo-100 font-medium">
          Your residency at <span className="text-white font-bold">Amani Heights</span> is active.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
          ...
          <div
            className={`p-6 rounded-3xl border backdrop-blur-md flex items-center justify-between ${tenant && tenant.balance <= 0 ? 'bg-emerald-400/20 border-emerald-400/30' : 'bg-amber-400/20 border-amber-400/30'}`}
          >
            <div>
              <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest mb-1">Status</p>
              <p className="text-xl font-black">
                {tenant && tenant.balance <= 0 ? 'Fully Paid' : 'Payment Due'}
              </p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
          ...
          <div className="space-y-4">
            {NOTICES.map((notice) => (
              <div
                key={notice.id}
                className={`p-5 rounded-3xl border ${notice.priority === 'Urgent' ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100'}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest ${notice.priority === 'Urgent' ? 'text-red-600' : 'text-slate-400'}`}
                  >
...
const QuickActionCard = ({
  ...
  <button
    onClick={onClick}
    className="flex flex-col items-center justify-center p-6 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group w-full"
  >
...
const StatCard = ({
  ...
  <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:border-indigo-100 transition-all">
    ...
    <div className="mt-4 flex items-center gap-1.5">
      ...
      <span className={`text-xs font-bold ${trendUp ? 'text-emerald-600' : 'text-amber-600'}`}>{trend}</span>
    </div>
  </div>
);
